// 앱 아이콘 아트 — SVG 문자열만 만든다(래스터화는 부르는 쪽에서).
// 컨셉: 앱 다크 테마(순검정 전광판 + 네온 그린 액센트)에 어울리는 "야구 베이스".
//   어두운 타일 위에 흰 베이스(정사각형)를 45° 다이아몬드로 아주 크게 → 위쪽 한 구석만
//   확대해 보여준다. 베벨(경사 테두리) + 네온 그린 발광 외곽선.
// 쓰는 곳: scripts/gen-icons.mjs(PWA·파비콘), scripts/gen-android-assets.mjs(APK 런처 아이콘).

export const NEON = '#4dff5e'; // --accent
export const RIM = '#bdf0c8'; // 베벨(경사진 테두리) — 밝은 연녹
export const FACE = '#f4fdf6'; // 윗면(밝은 흰)
...
/**
 * 베이스 한 구석 아이콘.
 * @param pad 여백 비율(마스커블/적응형 안전영역).
 * @param opts.bg 배경 채우기. `'none'`이면 배경 없이 그림만(적응형 아이콘 전경용).
 * @param opts.radius 배경 타일 모서리 반경(0이면 각진 정사각형).
 */
export function svg(pad = 0, { bg = BG, radius = 0.2 } = {}) {
  ...
  ${bg === 'none' ? '' : `<rect width="1" height="1" rx="${radius}" fill="${bg}"/>`}
