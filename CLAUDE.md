...
## 기능
- **헤더**: 좌측 세로로 `[밝기 토글] [? 게임 방법]`(원형), 가운데 타이틀(`숫자 야구 ⚾`)+부제,
  우측 세로로 혼자일 땐 `[새 게임] [멀티 ▶]`(멀티는 포인트색 강조 → `section='multi'`), 멀티일 땐 `[◀ 혼자]`.
  모드 전환은 이 헤더 버튼으로만(상단 혼자/멀티 탭 없음).
...
- **추측 기록**: 각 줄에 S·B·O를 고정 3칸으로 표시(합=자릿수, O=자릿수-S-B). 색은 야구 전광판 관례: S 주황 · B 초록 · O 빨강.
