...
## 기능
- **헤더**: 좌측 상단 `[? 게임 방법]`(원형), 가운데 타이틀(`숫자 야구 ⚾`)+부제,
  우측 세로로 혼자일 땐 `[새 게임] [멀티 ▶]`(멀티는 포인트색 강조 → `section='multi'`), 멀티일 땐 `[◀ 혼자]`.
  모드 전환은 이 헤더 버튼으로만(상단 혼자/멀티 탭 없음). 밝기 토글은 없음(단일 다크).
...
- **디자인 시스템**: **야구장 전광판(스코어보드)** 컨셉. 순검정 보드에 세븐세그먼트 LED 숫자.
  **단일 다크 테마**(전광판은 하나의 어두운 세계라 라이트/다크 토글 없음 → `ThemeToggle` 제거).
  ...
  - **`LampBank`**(`src/components/LampBank.tsx`): B·S·O 카운트 **전구 뱅크**. 직전 추측 판정을 전구로 점등
    (혼자=`App`, 스피드대결=`GuessBoard`). 게임 판정이 곧 스트라이크·볼·아웃이라 전광판 카운트등과 딱 맞물림.
