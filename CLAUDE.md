...
- **입력 칸**: `GameState.slots`(길이 digits, 빈 칸 ''). 칸을 탭하면 그 칸만 제자리에서 비워지고
  ...
  **길게 눌러 고정**(`toggleLock`, 솔로): 채워진 칸을 ~0.45초 길게 누르면 그 자리 고정(🔒·그린 링) — 제출해도 유지(`GameState.locked`), 탭 클리어 방지. 짧은 탭은 지우기.
...
- **메모**(분리 버튼): `Keypad`의 `markButtons` 모드 — 마크를 **분리 버튼 `[O △볼 …]`** = `[✕ O][△ B][○ S][↺초기화]`로 직접 고른다(옛 순환식 대체). 라벨은 영문 약자(O/B/S), 배지는 도형(✕/△/○).
  **마크 선택 = 토글**(`onPickMark`, 같은 걸 다시 누르면 해제=입력 모드). **숫자 탭 = 토글**(고른 표시 붙였다 뗌, `toggleMemoMark`). `↺`=메모 전체 초기화(`clearMemo`/`onClearMemo`).
  ...
  제출(추측) 버튼 라벨은 **던지기**(솔로). `MemoPad`는 입력칸 없는 메모 전용 키패드.
