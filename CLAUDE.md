...
- **기록 다시보기**(솔로 종료 후): 결과 카드 `[📜 기록 다시보기]` → 전광판에 `.review-bar`(← 결과 · 정답 · 📤) + `History stagger highlightLast`(행 순차 등장, 정답 행 강조). App `reviewOf`(=그 판의 `guesses` 참조라 새 판이면 자동 해제).
- **기록 공유**(`src/components/ShareSheet.tsx` + `src/share/recordCard.ts`): 결과 카드 `[📤 기록 공유]`/다시보기의 📤 → 시트. `drawRecordCard`가 **Canvas 2D로 직접**(라이브러리 없음) 1080px 카드 PNG를 그림(세븐세그먼트·S/B/O 전구, 현재 테마 토큰 사용, 최소 4:5·기록 많으면 세로로 늘어남).
