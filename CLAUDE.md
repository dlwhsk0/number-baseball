...
- **배경**(`src/components/FieldBackdrop.tsx`, `.field-bg`): 타자석에서 본 그라운드(잔디·내야·베이스·마운드·홈플레이트) SVG. 종횡비(400/700) 고정으로 균일 스케일, 일부러 화면 밖으로 넘치게. **마운드(투수석)가 항상 화면 정중앙**(`top:50%` + `translate(-50%,-67.1429%)`), 크기는 `width: min(220vw, 102dvh)` — 세로 기준 상한이라 창을 넓혀도 안 커진다. 위로 페이드(mask).
...
- **KBO 팬 랭킹**(구단 대항전): 팬이 응원 구단(KBO 10개 구단, `src/game/teams.ts` ↔ `server/src/teams.ts` 복제)을 달고 경쟁.
  ...
  **구단 표시는 이름이 아니라 공식 엠블럼**(`TeamChip` — 투명 배경 로고 그대로, `withName`이면 옆에 구단명).
  ...
  - **멀티 구단 대결**: 온라인 `create`/`join`에 `playerId`·`team` 동봉 → 판이 끝나면 **구단이 다른 참가자 쌍마다** 승/패/무 기록(`pairMatches`).
    주고받기=승자 vs 패자, 스피드=최종 순위 상위가 승(둘 다 미해결=무). 주고받기 **VS 매치업 카드엔 양쪽 구단 엠블럼 + 구단색(`Team.accent`) 테두리·발광**(`VsSide`). 같은 구단·구단 없음·같은 기기·추측 0회·중도 이탈은 제외. 로컬(패스앤플레이)은 기록 안 함.
    **제외는 UI로 알린다**(`.vs-sameteam`): 멀티 시작 메뉴에 한 줄(`.fan-note`), 그리고 매치업 연출에서 한 번 — 주고받기 VS 화면은 상대와 구단이 같을 때, 스피드 인트로는 서로 다른 구단이 둘 미만일 때. 로비·순위·VS 연출에 구단 칩.
  - **KBO 탭**(헤더 세그먼트 `section='kbo'`, `src/components/KboBoard.tsx` — 모달 아님, 인라인 화면·안쪽만 스크롤): 상단 **내 응원 구단 카드**(탭하면 구단 변경 — 설정 시트엔 구단 항목 없음, KBO 탭이 구단 변경의 기본 위치) — 솔로 순위(판 없으면 -)·대결 순위(승패무) 요약 + 탭 **[솔로](구단별 솔로 랭킹전 누적) · [대결](KBO식 승·패·무·승률·게임차)**.
    **[개인] 탭 = 한 판 평균 점수 순위**(실패 0점 포함, `PLACEMENT_GAMES`=10판 이상만 — 그 전은 **배치고사**, `rank=null`). 누적 합계는 판 수 싸움이라 버림.
    평균은 SQL에서 소수 둘째 자리 반올림 후 비교(`topPlayers`·`myRank` 같은 값 → 동순위 일치). 등급 `tierOf`(루키<6·유망주 6+·주전 8+·올스타 10+·MVP 12+, `TIERS`).
    탭 맨 위 **내 기록 카드**(`MyStat` — 배치 진행 전구 10칸 / 등급·평균·순위 + '다음 순위까지 +x점'(`me.aboveAvg`)·'다음 등급까지'), 목록엔 등급 배지. 내 구단 카드 3번째 칸 '내 순위'(배치면 n/10), 결과 카드 '내 평균 · 순위' 또는 '배치고사 n/10'(`RankedResult.avg/games`).
    avg가 없는 응답(옛 서버)은 개인 기록 없음으로 처리(배포 순서 무관하게 안 죽게). 스모크 `ranked-smoke.mjs`는 첫 판=배치 기준.
...
- **대결 모드**(App `section='multi'`): **멀티 시작 메뉴** — 하나의 글라스 카드(`.online-menu-card`)에 위→아래로
  ...
  **랜덤 매치**(주고받기 선택 시에만 `[🎲 랜덤 매치]`가 맨 위 주 버튼, 방 만들기는 보조로): `OnlineDuel` `entry.action='random'` → 서버 `queue`(자릿수별 대기열, 먼저 기다린 쪽이 방장=선공, 같은 기기 `playerId`끼리 제외, 취소는 `cancelQueue`(대기열만 — 60초 초과 시 성사와 겹쳐도 방을 안 나감)·`leave`, 끊기면 자동 제외) → 성사 시 서버가 방을 만들어 둘에게 `matched`(코드·자리·토큰·상대) + `phase` → 이후는 코드 방과 같은 흐름.
  **랜덤 방(`Room.random`)은 코드 입장 불가, 누가 나가든 방 종료**(방장 대기 복귀 없음) → 결과 화면에 `[🎲 새 상대 찾기]`. 60초(`MATCH_TIMEOUT_S`) 안에 상대가 없으면 "지금은 상대가 없어요" — `[방 만들어 초대하기]`/`[다시 찾기]`. 스피드 랜덤 매치는 미지원. 스모크 `server/test/random-match-smoke.mjs`.
  **코드로 입장은 모드 무관** — 먼저 `peek`(부수효과 없는 방 조회, `src/net/peek.ts`)으로 방장이 만든 모드·자릿수를 받아 그 모드로 자동 진입(`peeking` 중 "확인 중…").
  ...
  - **온라인 대결**: `src/versus/OnlineDuel.tsx` + `src/net/`(`socket.ts` 단일 소켓, `protocol.ts` 이벤트 타입).
    ...
    **관측**: 메트릭은 **게임 포트와 분리된 별도 포트 `METRICS_PORT`(기본 9091)** 의 `GET /metrics`(Prometheus, `server/src/metrics.ts` — 방 생성/입장/시작/종료/추측 카운터·활성방/소켓 게이지·기본 지표). **도메인(3001)엔 노출 안 함**(`도메인/metrics`=404) → Traefik 라우팅 밖이라 비공개, Prometheus는 Docker 내부에서 `<서비스>:9091` 스크레이프. + pino 구조화 로그(`server/src/logger.ts`, stdout→Dokploy 로그). env `LOG_LEVEL`·`METRICS_PORT`·`METRICS_TOKEN`.
    ...
    선공이 맞히면 후공 **역전 찬스**, 시작 발표, 상대 대기 랜덤 멘트, **내 숫자 peek**(블러+눌러서 토글 확인/숨김 — 턴바 안 `나가기` 왼쪽),
...
- **전광판 당겨서 펼치기**(솔로·온라인 주고받기·온라인 스피드 공용, `src/components/usePullExpand.ts`): 전광판 아래 손잡이(`.pull-handle`)를 **끌어내리면** 아래 박스(솔로 `.batter-wrap`, 온라인 `.duel-lower`)에 음수 margin-bottom을 줘 밀어내고 기록이 전체 높이 — 손가락 1:1 추적, 끝을 넘기면 고무줄 저항, 놓으면 거리(22%)·속도 기준으로 탄성 스냅(오버슈트 cubic-bezier). **다시 끌어올리면** 복귀, 탭=토글, 키보드 입력·게임 종료 시 자동 복귀. 타자석은 언마운트 X(입력·후보 유지, 펼친 동안 `inert`). 끈 뒤 손잡이 blur(Enter가 던지기로 가게). 솔로 기록은 `History followLatest`(새 추측·크기 변화 때 최신 행으로 스크롤).
  화면 세로 gap이 달라 `usePullExpand(gap)`로 넘긴다(솔로 `.app` 12 / 온라인 `.versus.play` 10). 온라인은 전광판이 가로 분할이라 기록 칸을 `.duel-cols`·`.sp-race-cols`로 한 겹 싸고 손잡이를 그 아래 둔다.
...
- **기록 공유**(`src/components/ShareSheet.tsx` + `src/share/recordCard.ts`): 결과 카드 `[📤 기록 공유]` → **아이폰 공유 시트 스타일**(아래에서 올라옴) — 미리보기 + 아이콘 4개 **[인스타 · X · 이미지 · 문구]**.
  - **인스타·X·이미지는 누르면 먼저 이미지를 무조건 다운로드**(인스타=스토리 9:16, X·이미지=게시물 4:5)한 뒤 아래 각자의 공유 방법으로 넘어간다(공유 시트 미지원·취소여도 다운로드는 됨) 파일명은 누른 시각 `homerun-YYYYMMDD-HHmmss.png`(`cardFileName` — 고정 이름이면 두 번째 저장부터 이름 충돌 창이 떠 인스타·X로 넘어가는 흐름을 막았다).
  - **인스타**: 스토리용 9:16 이미지를 파일 공유(모바일 시트 → 인스타). 웹에선 링크 스티커 자동 부착이 불가 → 누르는 순간 **링크를 클립보드에 복사**하고 "🔗 스티커에 붙여넣기" 안내. **X**: 꼬들처럼 문구가 채워진 트윗 작성 창(intent). **이미지**: 게시물 4:5 이미지(모바일=공유 시트의 '이미지 저장', 데스크톱=다운로드). **문구**: `buildShareText`(숫자 없이 🟠S🟢B🔴O 이모지 워들식 + URL) 복사.
  - `drawRecordCard(record, format, {logo})`: **Canvas 2D로 직접**(라이브러리 없음) 1080px PNG — 형식 `CARD_FORMATS`(story 1080×1920 UI 가림 영역 비움 / post 1080×1350), 기록 수에 맞춰 행 높이(40~124)·헤더 배율 자동, 넘치면 그때만 세로로 늘어남. 색은 **현재 테마 토큰**(응원 구단 테마 포함).
  - **구단 로고**: 응원 구단 테마 적용 중(App `rankedTeam`)이면 타이틀 옆 엠블럼 + 기록 보드 워터마크. 시트의 `[○○ 로고 넣기]` 스위치로 끌 수 있음(`localStorage.nb_share_logo`).
  - **링크 미리보기**: `index.html` OG/Twitter 메타 + `public/og-image.png`(1200×630). SW 프리캐시엔 안 넣음.
