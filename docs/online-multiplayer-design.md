...
- **내 숫자 peek** — 내가 정한 숫자를 기본 블러 처리, 눌러서 토글 확인/숨김(어깨너머 방지). 자리는 **턴바 안, `나가기` 왼쪽**(줄을 따로 쓰지 않아 전광판이 그만큼 넓다).
...
슬롯 높이는 **입력행 기준 92px — 솔로 타자석과 같은 값**(`.board .input-display`)이고, 상대 입력
미리보기도 그 안에 들어가게 맞춰뒀다. 한때 이 슬롯을 170px로 잡았는데 그건 '발표 카드가 잘리지
않을 높이'였고, 88px짜리 입력칸이 그걸 물려받아 위아래 41px씩 버려지며 전광판을 눌렀다. 지금은
**발표 카드만 `position:absolute`로 슬롯 밖(전광판 쪽)으로 띄운다** — 솔로의 `.solo-reveal`과 같은
방식이라 자리 고정 원칙은 그대로다.

전광판 아래에는 **당겨서 펼치는 손잡이**(`.pull-handle`)가 있다. 끌어내리면 `.duel-lower`가 아래로
밀려나고 양쪽 기록이 화면 전체를 쓴다(솔로와 같은 `usePullExpand`, gap만 10px로 넘긴다). 입력부는
언마운트하지 않아 입력·메모가 살아 있고, 펼친 동안엔 `inert`. 키보드를 치거나 판이 끝나면 자동 복귀.
